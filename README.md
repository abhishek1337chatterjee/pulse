...
| File | Role |
|---|---|
| `claude-stats/bin/ingest-daily.sh` | mirrors the window's JSONL with `usage.iterations` stripped (works around [ccusage#1710](https://github.com/ccusage/ccusage/issues/1710), which drops Fable 5.1 entries), then runs `npx ccusage claude daily --json` twice (plain + `--instances --breakdown`), upserts into `daily_usage` (PK: `date,model`) and `project_daily_usage` (PK: `project_path,date,model`). Both share an 8-day rolling-rewrite window so older rows are immutable; `project_daily_usage` rolled up sums to `daily_usage` to the cent. The `claude` subcommand is required under ccusage v19+; top-level `ccusage daily` now aggregates all agents and drops `modelBreakdowns`. |
