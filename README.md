...
| `claude-stats/bin/ingest-caveman.sh` | reads the caveman plugin's `~/.claude/.caveman-history.jsonl` into `caveman_sessions` (PK `session_id`, dedups the plugin's double-logged lines via `arg_max` by `ts`). Stores the plugin's `est_saved_tokens` for reference — the dashboard's caveman panel measures real `total_output_tokens` per assistant message, caveman-on vs caveman-off cohorts; the lifetime estimate sum appears only as labeled subtitle text, never charted. `est_saved_usd` is dropped (the plugin's price table lags new models). |
