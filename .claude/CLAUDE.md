...
- **Never revert `aggregate-daily.sh` to an unconditional DELETE+INSERT** — it runs after every 5-min poll and DuckDB never reclaims emptied row groups (file hit 4.9 GB on 2026-09-11). Keep the TEMP-table + `WHERE (SELECT changed FROM chg)` guard, keep `ROUND(SUM(...), 6)` (parallel SUM is nondeterministic in the last ulp), and keep `compact-db.sh` last in `battery-stats-aggregate.service`. Any new writer must take `/tmp/battery-stats-aggregate.lock`.
